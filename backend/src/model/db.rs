...
pub async fn init_db() -> Result<Db, sqlx::Error> {
  // Don't have permision on heroku
  // -- Create the db with PG_ROOT (dev only)
  // {
  //   let root_db = new_db_pool(PG_HOST, PG_ROOT_DB, PG_ROOT_USER, PG_ROOT_PWD, PG_APP_MAX_CON).await?;
  ...
  new_db_pool(PG_HOST, PG_APP_DB, PG_APP_USER, PG_APP_PWD, 1).await
}
